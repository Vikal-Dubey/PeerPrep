  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-5 flex flex-col gap-5 text-text shadow-xl font-display">
      <div className="select-none">
        ...
        <p className="text-muted text-[10px] font-mono mt-1 uppercase tracking-wider">Upload candidate resume PDF for compliance scanning</p>
      </div>
        ...
        >
          ...
          {file ? (
            <div className="text-center">
              <p className="text-xs font-semibold text-text font-mono truncate max-w-[220px]" title={file.name}>
                {file.name}
              </p>
            ...
            <div className="text-center select-none">
              ...
              <p className="text-[10px] text-muted/65 font-mono mt-1 uppercase tracking-wider">Maximum file size: 5MB</p>
            </div>
          )}
        </div>

        {error && (
          <p className="text-error text-[10px] font-mono leading-normal bg-error/5 border border-error/20 px-3 py-1.5 rounded-md">
            {error}
          </p>
        )}
        ...
        {file && (
          <div className="flex gap-2 select-none">
            <button
              ...
              disabled={analyzing}
              className="flex-1 bg-accent-cool text-bg font-bold text-xs py-2.5 rounded-lg hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-accent-cool/10 cursor-pointer"
            >
              {analyzing ? (
      ...
      {result && (
        <div className="border-t border-border/80 pt-4 flex flex-col gap-4 animate-fadeIn">
          {/* File Name & Score */}
          <div className="flex flex-col gap-2.5 bg-bg/50 border border-border rounded-lg p-4">
            <div className="flex justify-between items-center gap-4">
              <span className="text-xs font-bold truncate max-w-[180px] font-mono text-text/95" title={uploadedFileName}>
                📄 {uploadedFileName}
              </span>
              <span className={`text-xs font-mono font-bold ${getScoreColor(result.score).split(" ")[1]}`}>
